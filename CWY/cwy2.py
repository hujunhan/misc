def main():
    user_input=input('Type a number, program would show you prime number that smaller than the number you type in: ')
    ...
    for i in range(2,n): # iterate every number from 2 to n-1
        ...
        if is_prime:
            print(i,end=' ') # don't make new line everytime, so the end is a space ' '
            count+=1
            if count==8:
                count=0
                print()
