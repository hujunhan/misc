...
x_ticks_label=[datetime.fromordinal(min_x+all_days*i//4).strftime('%Y-%m-%d') for i in range(5)]
...
ax.text(0.5, 0.5, 'y=%.1fx+(%.1f)'%(np.polyfit(x, y, 1)[0],np.polyfit(x, y, 1)[1]), transform=ax.transAxes)
